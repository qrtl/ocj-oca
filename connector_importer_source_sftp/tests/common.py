...
class TestSourceCSVSFTPMixin:
    ...
    def _setup_source_records(cls):
        ...
        # `backend_type` and `sftp_port` are env-computed (server.env.mixin)
        # fields that declare a default: on create the default of their
        # companion `x_*_env_default` field wins over the value passed here,
        # so the backend would fall back to the filesystem adapter. Writing
        # them after create stores the values through the inverse.
        cls.storage_backend.write({"backend_type": "sftp", "sftp_port": 2222})
